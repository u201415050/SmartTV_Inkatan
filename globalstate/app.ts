...
class State {
  ...
  constructor(name) {
    ...
    this.mapblockedMessages = {};
  }
  addBlockedMessage(message) {
    this.mapblockedMessages[message] = true;
    return true;
  }
  removeBlockedMessage(message) {
    delete this.mapblockedMessages[message];
    return true;
  }
...
class GlobalState {
  ...
  execute(actionObject) {
    ...
    try {
      if (this.mapBlockedActions[actionObject["action"]] == null ||  this.stateControllers[
